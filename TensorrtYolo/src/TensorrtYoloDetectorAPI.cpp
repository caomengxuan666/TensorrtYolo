...
struct InternalDetection {
  ...
  float score;
  cv::Rect box;
  bool operator<(const InternalDetection &other) const {
...
public:
  ...
  static std::vector<InternalDetection>
  toInternal(const std::vector<Detection> &detections) {
    std::vector<InternalDetection> result;
    result.reserve(detections.size());
    for (const auto &det : detections) {
      result.push_back({det.class_id, det.score,
  ...
  fromInternal(const std::vector<InternalDetection> &internalDets) {
    std::vector<Detection> result;
    result.reserve(internalDets.size());
    for (const auto &det : internalDets) {
      result.push_back({det.classId, det.score, det.box.x, det.box.y,
...
public:
  TensorrtEngineManager(Logger &logger, const std::string &engineFile)
      : logger_(logger), input_w_(640), input_h_(640), num_boxes_(8400) {
  ...
  void infer(const std::vector<float> &inputData,
...
private:
  ...
  void loadEngine(const std::string &engineFile) {
    ...
    std::ifstream file(engineFile, std::ios::binary |
                                       std::ios::ate); // 打开并定位到文件末尾
    if (!file.is_open()) {
      throw std::runtime_error("Failed to open engine file: " + engineFile +
                               " (check if path is correct or file exists)");
    }
    ...
    if (fileSize <= 0) {
      file.close();
      throw std::runtime_error("Engine file is empty or corrupted: " +
                               engineFile);
    }
    ...
    if (!file.read(engineData.data(), fileSize)) {
      ...
      throw std::runtime_error(
          "Failed to read engine file: " + engineFile +
          " (file may be corrupted or read permission denied)");
    }
    file.close();
    ...
    if (!runtime_) {
      throw std::runtime_error(
          "Failed to create TensorRT runtime (check TensorRT installation)");
    }
    ...
    if (!engine_) {
      throw std::runtime_error(
          ...
          " (file may be corrupted, incompatible with current TensorRT "
          "version, or not a valid engine file)");
    }

    // 7. 创建推理上下文
    context_.reset(engine_->createExecutionContext());
    if (!context_) {
      throw std::runtime_error("Failed to create inference context "
                               "(insufficient GPU memory or invalid engine)");
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Failed to parse input/output tensors from engine (engine file may "
  ...
  using RuntimePtr =
      std::unique_ptr<IRuntime, decltype([](IRuntime *p) { delete p; })>;
  using EnginePtr =
      std::unique_ptr<ICudaEngine, decltype([](ICudaEngine *p) { delete p; })>;
  using ContextPtr =
      std::unique_ptr<IExecutionContext,
                      decltype([](IExecutionContext *p) { delete p; })>;
...
public:
  ...
  void preprocess(const unsigned char *bgrData, int srcWidth, int srcHeight,
                  int dstWidth, int dstHeight, std::vector<float> &outputData,
                  float &scale, int &dw, int &dh) {
    // 转换为OpenCV矩阵（不拷贝数据，仅包装指针）
    cv::Mat img(srcHeight, srcWidth, CV_8UC3,
                const_cast<unsigned char *>(bgrData));
    ...
    // 缩放并填充图像
    cv::Mat resized,
        padded(dstHeight, dstWidth, CV_8UC3, cv::Scalar(114, 114, 114));
    ...
    for (int c = 0; c < 3; ++c) { // BGR三通道
      std::for_each(std::execution::par, padded.begin<cv::Vec3b>(),
                    padded.end<cv::Vec3b>(), [&](cv::Vec3b &pixel) {
                      int y = &pixel - padded.ptr<cv::Vec3b>(0); // 计算行索引
                      int x = y % dstHeight;                     // 计算列索引
                      y /= dstHeight;
                      outputData[c * dstHeight * dstWidth + y * dstWidth + x] =
                          pixel[c] / 255.0f;
                    });
    }
  }
...
public:
  void preprocess(const unsigned char *bgrData, int width, int height,
                  int dstWidth, int dstHeight, std::vector<float> &outputData) {
  }
};
...
private:
  // 非极大值抑制（NMS）
  std::vector<InternalDetection>
  nonMaximumSuppression(std::vector<InternalDetection> &detections) {
    if (detections.empty())
      return {};
    ...
    std::transform(std::execution::par, detections.begin(), detections.end(),
                   areas.begin(), [](const InternalDetection &d) {
    ...
    for (size_t i = 0; i < detections.size(); ++i) {
      ...
      std::for_each(std::execution::par_unseq, detections.begin() + i + 1,
                    detections.end(), [&](const InternalDetection &d) {
                      ...
                      int xx2 = std::min(currentBox.x + currentBox.width,
                                         d.box.x + d.box.width);
                      int yy2 = std::min(currentBox.y + currentBox.height,
                                         d.box.y + d.box.height);
                      int interArea =
                          std::max(0, xx2 - xx1) * std::max(0, yy2 - yy1);
                      float iou =
                          interArea / (currentArea + areas[j] - interArea);
...
public:
  explicit ClassificationPostprocessor(int num_classes)
      : num_classes_(num_classes) {}

  std::vector<Detection> process(const std::vector<float> &outputData) {}
  ...
  void drawInternal(const unsigned char *bgrData, int width, int height,
    ...
    for (const auto &d : detections) {
      ...
      cv::line(img, cv::Point(box.x + radius, box.y),
               cv::Point(box.x + box.width - radius, box.y), color, 2,
               cv::LINE_AA);
      cv::line(img, cv::Point(box.x + box.width, box.y + radius),
               cv::Point(box.x + box.width, box.y + box.height - radius), color,
               2, cv::LINE_AA);
      cv::line(img, cv::Point(box.x + radius, box.y + box.height),
               cv::Point(box.x + box.width - radius, box.y + box.height), color,
               2, cv::LINE_AA);
      cv::line(img, cv::Point(box.x, box.y + radius),
               cv::Point(box.x, box.y + box.height - radius), color, 2,
               cv::LINE_AA);
      ...
      // 绘制文本
      cv::putText(img, label, cv::Point(box.x + 4, box.y - 4), fontFace,
  ...
  void drawExternal(const unsigned char *bgrData, int width, int height,
    ...
    for (const auto &det : detections) {
      ...
      std::string label = "Class: " + std::to_string(det.class_id) +
                          " Score: " + std::to_string(int(det.score * 100)) +
                          "%";
      ...
      cv::rectangle(image, cv::Point(box.x, box.y - textSize.height - baseline),
                    cv::Point(box.x + textSize.width, box.y), color, -1,
                    cv::LINE_AA);

      // 绘制文本
      cv::putText(image, label, cv::Point(box.x, box.y - baseline), fontFace,
                  fontScale, cv::Scalar(255, 255, 255), thickness, cv::LINE_AA);
    }
  }

  // 使用回调或默认可视化
  void visualizeWithCallback(const unsigned char *bgrData, int width,
                             int height,
                             const std::vector<Detection> &detections) {
    if (callback_) {
...
public:
  void visualize(const unsigned char *bgrData, int width, int height,
                 const std::vector<Classification> &classifications);
};

// -------------------------- 核心实现类（协调各模块）
// --------------------------
enum class TaskMode { DETECTION, CLASSIFICATION };
...
public:
  ...
  explicit Impl(const DetectorConfig &config)
      : logger_(), engineManager_(logger_, config.getEngineFile()),
        preprocessor_(), postprocessor_(config), visualizer_() // 直接传入配置类
        ,
        config_(config) {}

  // 检测流程：预处理->推理->后处理->转换结果
  std::vector<InternalDetection> detect(const unsigned char *bgrData, int width,
                                        int height) {
    // 1. 预处理
    float scale;
    int dw, dh;
    ...
    preprocessor_.preprocess(
        bgrData, width, height, engineManager_.inputWidth(),
        engineManager_.inputHeight(), inputData, scale, dw, dh);
...
TensorrtYoloDetectorAPI::detect(const unsigned char *bgrData, int width,
                                int height) {
  auto internalDets = pImpl_->detect(bgrData, width, height);
  return DetectionConverter::fromInternal(internalDets); // 转换为外部结果
}

void TensorrtYoloDetectorAPI::visualize(
    unsigned char *bgrData, int width, int height,
    const std::vector<Detection> &detections) {
  auto internal = DetectionConverter::toInternal(detections);
  pImpl_->visualize(bgrData, width, height, internal);
}

void TensorrtYoloDetectorAPI::registerVisualizationCallback(
    VisualizationFunction callback, void *userData) {
  pImpl_->registerVisualizationCallback(callback, userData);
}

void TensorrtYoloDetectorAPI::visualizeWithCallback(
    unsigned char *bgrData, int width, int height,
    const std::vector<Detection> &detections) {
