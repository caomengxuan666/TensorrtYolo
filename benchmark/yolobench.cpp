...
std::vector<Detection> nonMaximumSuppression(std::vector<Detection> &detections,
  ...
  for (size_t i = 0; i < detections.size(); ++i) {
    ...
    std::for_each(std::execution::par, detections.begin() + i + 1,
                  detections.end(), [&](const Detection &det_j) {
...
loadEngineFileAsync(const std::string &engineFile) {
...
// Preprocess
void preprocessParallel(const cv::Mat &img, float *dst, int input_w,
                        int input_h, float &scale, int &dw, int &dh) {
  // 参数计算（不变）
  int w = img.cols, h = img.rows;
  scale = std::min(input_w / static_cast<float>(w),
                   input_h / static_cast<float>(h));
...
static void BM_TensorRT_FullPipeline(benchmark::State &state) {
  ...
  // 累计各阶段耗时
  double total_pre = 0, total_h2d = 0, total_infer = 0, total_d2h = 0,
         total_post = 0, total_nms = 0;
  int rounds = 0;
  ...
  for (auto _ : state) {
    ...
    cudaMemcpyAsync(d_input, inputData.data(), inputSize,
    ...
    cudaMemcpyAsync(outputData.data(), d_output, outputSize,
    ...
    for (int t = 0; t < num_threads; ++t) {
      detection_futures.push_back(std::async(std::launch::async, [&, t] {
        ...
        const int end =
            (t == num_threads - 1) ? NUM_BOXES : (t + 1) * chunk_size;
        ...
        for (int i = start; i < end; ++i) {
          ...
          local_dets.push_back({0, score,
                                cv::Rect(cv::Point(int(x1), int(y1)),
                                         cv::Point(int(x2), int(y2)))});
        }
        std::lock_guard<std::mutex> lock(mtx);
        detections.insert(detections.end(), local_dets.begin(),
                          local_dets.end());
      }));
    }
    ...
    // 每轮打印
    std::cout << "[BENCH] preprocess: " << pre << " ms, H2D: " << h2d
              << " ms, infer: " << infer << " ms, D2H: " << d2h
              << " ms, post: " << post << " ms, NMS: " << nms << " ms"
              << std::endl;
  ...
  // 平均统计
  if (rounds > 0) {
    double sum = total_pre + total_h2d + total_infer + total_d2h + total_post +
                 total_nms;
    std::cout << "\n==== 平均每轮耗时(ms) ====" << std::endl;
    std::cout << "preprocess: " << total_pre / rounds << " ("
              << (total_pre / sum * 100) << "%)" << std::endl;
    std::cout << "H2D:        " << total_h2d / rounds << " ("
              << (total_h2d / sum * 100) << "%)" << std::endl;
    std::cout << "infer:      " << total_infer / rounds << " ("
              << (total_infer / sum * 100) << "%)" << std::endl;
    std::cout << "D2H:        " << total_d2h / rounds << " ("
              << (total_d2h / sum * 100) << "%)" << std::endl;
    std::cout << "post:       " << total_post / rounds << " ("
              << (total_post / sum * 100) << "%)" << std::endl;
    std::cout << "NMS:        " << total_nms / rounds << " ("
              << (total_nms / sum * 100) << "%)" << std::endl;
